...
exports.NodeWrapper = function (_a) {
    ...
    var onValInput = React.useCallback(function (e) {
        if (!config.readonly) {
            e.stopPropagation();
            if (!isDragging.current) {
                onNodeInput({ config: config, nodeId: node.id, value: e.target.value });
            }
        }
    }, [config, node.id]);
    ...
    var children = (React.createElement(React.Fragment, null,
        React.createElement(react_resize_observer_1.default, {
            onResize: function (rect) {
                var newSize = { width: rect.width, height: rect.height };
                setSize(newSize);
            }
        }),
        React.createElement(NodeInner, { node: node, config: config, onValInput: onValInput }),
